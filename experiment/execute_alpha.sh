...
TOTEST_L="0.125"
TOTEST_LARGE_N="256" 
TOTEST_LARGER_N="512" # unused in this experiment 
TOTEST_SMALL_N="1"
TOTEST_EPS="0.02094" # 2^-7
TOTEST_2LOGALP="3 2 1 0 -1 -2 -3 -4 -5 -6" # -8" # 2^-2 ... 2^-8
TOTEST_THETA="0.0"
TOTEST_CONT="100"
TOTEST_VFFILE="advection_diffusion_msfem.idp"
TOTEST_OSCOEF="0"
...
for TEST_LARGE_N in $TOTEST_LARGE_N; do sed -i "s/N=.*/N= $TEST_LARGE_N/" "experiment/parameters.txt"
    if [ $TEST_2LOGALP = "-6" ] # the last test requires a finer reference solution (skipped here)
