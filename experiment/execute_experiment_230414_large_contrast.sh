...
TOTEST_VFFILE="advection_diffusion_direct.idp advection_diffusion_skew_symmetric.idp advection_diffusion_msfem_supg.idp advection_diffusion_msfem.idp advection_diffusion_p1.idp advection_diffusion_p1_supg.idp"
TOTEST_L="1."
TOTEST_EPS="0.020944" # 2^-7
TOTEST_2LOGALP="2 1 0 -1 -2 -3 -4 -5 -6 -7"
TOTEST_SWITCHALPHA="-7" # value for which the mesh size TOTEST_LARGER_N should be used for the fine mesh
TOTEST_THETA="0.15"
TOTEST_CONT="100"
TOTEST_LARGE_N="2048" 
...
for TEST_LARGE_N in $TOTEST_LARGE_N; do sed -i "s/N=.*/N= $TEST_LARGE_N/" "experiment/parameters.txt"
    if [ $TEST_2LOGALP = $TOTEST_SWITCHALPHA ] # the last test requires a finer reference solution (skipped here)
