...
TOTEST_L="1."
TOTEST_LARGE_N="16 32" 
TOTEST_LARGER_N="256" 
LARGER_N_THR="-8" # a larger value of N can be used when 2logalp is below a certain threshold
TOTEST_SMALL_N="16"
TOTEST_EPS="0.25"
TOTEST_2LOGALP="-2"
TOTEST_THETA="0.15"
...
TOTEST_OSCOEF="0 1.2"
TOTEST_GLUE="dof" # either "dof" or "restrict" -- without OS, this option is automatically ignored
TOTEST_STR_DIR="0"
TOTEST_USE_B="0" # it is important to treat bubbes first, so the offline stages without bubbles can be loaded
TOTEST_TREAT_B="out_system" # either "out_system" or "in_system" -- without bubbles, this option is automatically ignored
...
## - It is more efficient to use bubbles in_system before using out_system
...
for TEST_LARGE_N in $TOTEST_LARGE_N; do sed -i "s/N=.*/N= $TEST_LARGE_N/" "experiment/parameters.txt"
    if (( $(echo "$TEST_2LOGALP <= $LARGER_N_THR" | bc) ))
